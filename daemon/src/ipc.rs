...
pub enum ServerMessage {
    ...
    Researching,
    /// A visible chunk of the assistant's reasoning/process -- what it
    /// found while researching, and (once available) the raw text the
    /// model produced before/around the structured plan JSON. Purely
    /// informational, never actionable -- shown so the user can see what
    /// led to a given plan or reply instead of only seeing the end result.
